...
agent1 = Agent(state_count, action_count, head_count,
               name="{}_l".format(name), steps_before_learning=1000, epsilon_decay=epsilon_decay,
               memory_capacity=100000, shared_exp=shared_exp, mode="simple", save_interval=1e5, #save_big_path="models/",
               save_memory_multiplier=0.1, epsilon=epsilon_max, epsilon_min=epsilon_min)
agent2 = Agent(state_count, action_count, head_count,
               name="{}_r".format(name), steps_before_learning=1000, epsilon_decay=epsilon_decay,
               memory_capacity=100000, shared_exp=shared_exp, mode="simple", save_interval=1e5, #save_big_path="models/",
               save_memory_multiplier=0.1, epsilon=epsilon_max, epsilon_min=epsilon_min)
...
def train_and_eval_multi(training_steps, test_interval, test_episodes, selector, environments, render=False,
...
train_and_eval_multi(5e5, 5e4, 10, selector_specific, environments, render=False, verbose=True)
